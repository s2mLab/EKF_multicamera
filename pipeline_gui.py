...
class PipelineTab(CommandTab):
    def __init__(self, master):
        ...
        for text, var in [
            ...
            ttk.Checkbutton(checks, text=text, variable=var).pack(side=tk.LEFT, padx=(0, 12))
...
class DualAnimationTab(CommandTab):
    def __init__(self, master, state: SharedAppState):
        ...
        ttk.Checkbutton(row2, text="Show trunk local frames", variable=self.show_trunk_frames_var, command=self.refresh_preview).pack(side=tk.LEFT)
        ttk.Label(row2, text="Use mouse drag in the preview to rotate the 3D view.").pack(side=tk.LEFT, padx=(12, 0))
...
class MultiViewTab(CommandTab):
    def __init__(self, master, state: SharedAppState):
        ...
        ttk.Label(row, text="Crop mode", width=10).pack(side=tk.LEFT)
        self.crop_mode = tk.StringVar(value="pose")
        ttk.Combobox(row, textvariable=self.crop_mode, values=["full", "pose"], width=10, state="readonly").pack(side=tk.LEFT, padx=(0, 6))
...
class FiguresTab(CommandTab):
    def __init__(self, master):
        ...
        ttk.Label(row, text="EKF 2D variant", width=16).pack(side=tk.LEFT)
        self.variant = tk.StringVar(value="acc")
        ttk.Combobox(row, textvariable=self.variant, values=["acc", "flip_acc", "both"], width=12, state="readonly").pack(side=tk.LEFT)
...
class AnalysisTab(CommandTab):
    def __init__(self, master):
        ...
        ttk.Checkbutton(form, text="interactive", variable=self.interactive_var).pack(anchor="w", padx=8, pady=4)
...
class ProfilesTab(CommandTab):
    def __init__(self, master, state: SharedAppState):
        ...
        ttk.Label(header, text="Family", width=8).pack(side=tk.LEFT)
        self.family = tk.StringVar(value="ekf_2d")
        ttk.Combobox(header, textvariable=self.family, values=["pose2sim", "triangulation", "ekf_3d", "ekf_2d"], width=14, state="readonly").pack(side=tk.LEFT)
        ...
        ttk.Checkbutton(self.common_frame, text="initial-rotation-correction", variable=self.initial_rot_var).pack(side=tk.LEFT, padx=(0, 12))
        ttk.Checkbutton(self.common_frame, text="no-root-unwrap", variable=self.unwrap_var).pack(side=tk.LEFT)
        ...
        ttk.Checkbutton(self.flip_frame, text="flip left/right", variable=self.flip_var).pack(side=tk.LEFT, padx=(0, 12))

        self.ekf2d_frame = ttk.Frame(form)
        ttk.Label(self.ekf2d_frame, text="Predictor", width=10).pack(side=tk.LEFT)
        self.predictor = tk.StringVar(value="acc")
        ttk.Combobox(self.ekf2d_frame, textvariable=self.predictor, values=["acc", "dyn"], width=8, state="readonly").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(self.ekf2d_frame, text="3D source", width=10).pack(side=tk.LEFT)
        ...
        ttk.Checkbutton(self.ekf2d_frame, text="dof_locking", variable=self.lock_var).pack(side=tk.LEFT)
...
class RootKinematicsTab(ttk.Frame):
    def __init__(self, master, state: SharedAppState):
        ...
        row.pack(fill=tk.X, padx=8, pady=4)
        ttk.Label(row, text="Famille", width=10).pack(side=tk.LEFT)
        self.family = tk.StringVar(value="rotations")
        ttk.Combobox(row, textvariable=self.family, values=["translations", "rotations"], width=14, state="readonly").pack(side=tk.LEFT, padx=(0, 6))
        ttk.Label(row, text="Quantité", width=10).pack(side=tk.LEFT)
        self.quantity = tk.StringVar(value="q")
        ttk.Combobox(row, textvariable=self.quantity, values=["q", "qdot"], width=10, state="readonly").pack(side=tk.LEFT, padx=(0, 6))
        ttk.Label(row, text="Rot unit", width=10).pack(side=tk.LEFT)
        ...
        ttk.Checkbutton(row2, text="unwrap rotations", variable=self.unwrap_var).pack(side=tk.LEFT)
        ttk.Checkbutton(row2, text="recalcul matrice + re-extraction Euler", variable=self.reextract_var).pack(side=tk.LEFT, padx=(12, 0))
        ttk.Checkbutton(row2, text="qdot par différence finie", variable=self.fd_qdot_var).pack(side=tk.LEFT, padx=(12, 0))
...
class JointKinematicsTab(ttk.Frame):
    def __init__(self, master, state: SharedAppState):
        ...
        ttk.Checkbutton(row, text="qdot par différence finie", variable=self.fd_qdot_var).pack(side=tk.LEFT)
...
class DDTab(ttk.Frame):
    def __init__(self, master, state: SharedAppState):
        ...
        ttk.Label(row1, text="Smooth (s)", width=10).pack(side=tk.LEFT)
        self.smoothing_window_s = tk.StringVar(value="0.15")
        ttk.Entry(row1, textvariable=self.smoothing_window_s, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(row1, text="Thr ratio", width=10).pack(side=tk.LEFT)
        self.height_threshold_ratio = tk.StringVar(value="0.20")
        ttk.Entry(row1, textvariable=self.height_threshold_ratio, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(row1, text="Thr abs", width=8).pack(side=tk.LEFT)
        self.height_threshold_abs = tk.StringVar(value="")
        ttk.Entry(row1, textvariable=self.height_threshold_abs, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(row1, text="Airtime (s)", width=11).pack(side=tk.LEFT)
        self.min_airtime_s = tk.StringVar(value="0.25")
        ttk.Entry(row1, textvariable=self.min_airtime_s, width=8).pack(side=tk.LEFT)

        row2 = ttk.Frame(controls)
        row2.pack(fill=tk.X, padx=8, pady=4)
        ttk.Label(row2, text="Min gap (s)", width=12).pack(side=tk.LEFT)
        self.min_gap_s = tk.StringVar(value="0.08")
        ttk.Entry(row2, textvariable=self.min_gap_s, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(row2, text="Prominence (m)", width=13).pack(side=tk.LEFT)
        self.min_peak_prominence_m = tk.StringVar(value="0.35")
        ttk.Entry(row2, textvariable=self.min_peak_prominence_m, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(row2, text="Contact win (s)", width=13).pack(side=tk.LEFT)
        self.contact_window_s = tk.StringVar(value="0.35")
        ttk.Entry(row2, textvariable=self.contact_window_s, width=8).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(row2, text="Angles", width=8).pack(side=tk.LEFT)
